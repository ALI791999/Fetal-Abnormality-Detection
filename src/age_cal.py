...
def ga_weeks_from_hc_intergrowth(hc_mm: float) -> float:
    """INTERGROWTH-21st HC-only dating. Input HC in mm, returns GA in weeks."""
    if hc_mm <= 0:
        raise ValueError("HC must be positive (mm).")
    ga_days = math.exp(0.05970 * (math.log(hc_mm))**2 + 6.409e-9 * (hc_mm**3) + 3.3258)
    ga_weeks = ga_days / 7.0
    if ga_weeks > configs.GA_UPPER_LIMIT or ga_weeks < configs.GA_LOWER_LIMIT:
        raise ValueError(
            f"GA {ga_weeks:.2f} weeks is outside limits "
            f"[{configs.GA_LOWER_LIMIT}, {configs.GA_UPPER_LIMIT}]"
        )
    return ga_weeks
